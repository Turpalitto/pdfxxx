...
- Process runner использует progress, execution routing и result shaping metadata через `client/src/tools/shared/process.ts`; callback-progress slug, строковые `WorkerOp`, ручные text/html/json/markdown/split/audio/metadata adapters и обходные main-thread paths не должны дублироваться в `tool-page.tsx`.
- Полная миграция каталога возможна позже маленькими PR/раундами.

**Правило:** Не плодить новые ручные списки output/worker/search/download/progress metadata, строковые worker op, ручные text-like/split/audio/metadata result adapters или обходные execution paths в компонентах; добавлять стабильные metadata в `client/src/tools/types.ts` и `client/src/tools/registry.ts`, а runtime-only исключения держать в маленьких тестируемых shared helpers.
